...
func (r *PostPostgres) Create(userId uuid.UUID, post listing.Post) (uuid.UUID, error) {
	var id uuid.UUID
	query := fmt.Sprintf("INSERT INTO %s (author_id, name, surname, role, education, additional, created_at, updated_at, photo) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id", postsTable)
	row := r.db.QueryRow(query, userId, post.Name, post.Surname, post.Role, post.Education, post.Additional, time.Now(), time.Now(), "photo")
...
func (r *PostPostgres) GetAll() ([]listing.Post, error) {
	...
	query := fmt.Sprintf("SELECT posts.id, posts.author_id, posts.name, posts.surname, posts.role, posts.education, posts.additional, posts.photo, posts.created_at, posts.updated_at FROM %s", postsTable)
...
func (r *PostPostgres) GetById(id uuid.UUID) (listing.Post, error) {
	...
	query := fmt.Sprintf("SELECT posts.id, posts.author_id, posts.name, posts.surname, posts.role, posts.education, posts.additional, posts.photo, posts.created_at, posts.updated_at FROM %s WHERE posts.id = $1", postsTable)
